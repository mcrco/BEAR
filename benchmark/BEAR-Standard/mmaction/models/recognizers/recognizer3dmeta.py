...
class Recognizer3DMeta(BaseRecognizer):
    ...
    def extract_feat(self, imgs, img_metas):
        ...
        if (hasattr(self.backbone, 'features')
            ...
            x = self.backbone.features(imgs, img_metas)
        elif self.backbone_from == 'timm':
            x = self.backbone.forward_features(imgs, img_metas)
        else:
            x = self.backbone(imgs, img_metas)
        return x
    ...
    def forward_train(self, imgs, labels, **kwargs):
        ...
        x = self.extract_feat(imgs, kwargs['video_name'])
        if self.with_neck:
