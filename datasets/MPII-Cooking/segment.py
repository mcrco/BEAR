...
raw_annotation_csv = open('detectionGroundtruth-1-0.csv', 'r')
...
for video_name in video_listdir:
    ...
    num_files_in_path = len(os.listdir(frame_path))
    if num_files_in_path == num_frames:
        print(frame_path, 'already completed')
        continue
...
for line in tqdm(raw_csv_reader):
    ...
    for index in range(start, end + 1):
        ...
        target_file = os.path.join(target_file_path, str(index).zfill(5) + '.jpg')
